...
SITE74_HOST="site74.wp1.local"
SITE82_HOST="site82.wp1.local"
...
# IMPORTANT: reachable from docker containers (NOT 127.0.0.1)
DB_HOST="${VM_IP}"
...
# MySQL listen for Docker containers
...
# WP-CLI cache (prevents mkdir permission + speeds up)
mkdir -p /tmp/wp-cli-cache
chown -R www-data:www-data /tmp/wp-cli-cache

# ---- Idempotent WordPress install function ----
wp_install() {
  local DOCROOT="$1"
  local DBNAME="$2"
  local URL="$3"
...
wp_install "$SITE74_ROOT" "$DB74" "http://${SITE74_HOST}" "WP1 Site74 (PHP 7.4)"
wp_install "$SITE82_ROOT" "$DB82" "http://${SITE82_HOST}" "WP1 Site82 (PHP 8.2)"
